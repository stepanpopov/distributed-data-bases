...
def hotel_detail(hotel_id):
    ...
    return render_template("hotel_detail.html", hotel=hotel, rooms=rooms, amenities=amenities)
...
def process_payment_html():
    try:
        ...
        flash(
            f"Оплата прошла успешно! Сумма: {result.get('amount_paid', amount_float)} руб.",
            "success",
        )
        return redirect(url_for("index"))
...
def reception_desk(hotel_id):
    ...
    return render_template("reception.html", hotel=hotel, reservations=reservations)
...
def register_guests_html():
    try:
        ...
        result = booking_service.register_guests(int(reservation_id), int(room_id), [int(gid) for gid in guest_ids])
...
def reception_city(city_name):
    try:
        ...
        return render_template("reception_city.html", city_name=city_name, reservations=reservations_list)
    except Exception as e:
...
def check_availability_api():
    ...
    result = availability_service.check_room_availability(int(hotel_id), int(room_category_id), start_date, end_date)
...
def get_available_rooms_api(hotel_id):
    ...
    rooms = availability_service.find_available_rooms(hotel_id, int(room_category_id), start_date, end_date)
